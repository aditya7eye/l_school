...
<form class="forms-sample" action="{{ url('update_leave_count') }}" method="get">
    ...
    <div class="form-group">
        ...
        <input type="text" maxlength="2" class="form-control required numberOnly" name="cl" value="{{ $emp_type->cl }}" id="cl" />
    </div>
    <div class="form-group">
        <label for="exampleInputEmail3">ML</label>
        <input type="text" maxlength="2" class="form-control required numberOnly" name="ml" value="{{ $emp_type->ml }}" id="ml"
               placeholder="ML">
    </div>
