...
//            $holidays = DB::select("SELECT * FROM `holiday` WHERE date BETWEEN '$year-$month-01' and '$year-$month-31'");//2;
            ...
            foreach ($employee_list as $employee) {
                ...
                if (!isset($parole_check)) {
                    ...
                    $taken_cl = $given_max_cl - isset($employee_leave_left) ? $employee_leave_left->cl : 0;//EmployeeLeaves::where(['employee_id' => $employee->id, 'leave_type' => 'CL', 'session_id' => $session_master->id])->count();
                    $taken_ml = $given_max_ml - isset($employee_leave_left) ? $employee_leave_left->ml : 0;//EmployeeLeaves::where(['employee_id' => $employee->id, 'leave_type' => 'ML', 'session_id' => $session_master->id])->count();
...
//                dd($td."<br>".$total_deduction."<br>".$total_gatepass."<br>".$payout."<br>");
    ...
    public function generate_payrole_pre_f(Request $request)
    {
        $employee_id = $request->input('employee_id');
...
//            $holidays = DB::select("SELECT * FROM `holiday` WHERE date BETWEEN '$year-$month-01' and '$year-$month-31'");//2;
            ...
            foreach ($employee_list as $employee) {
                ...
                if (!isset($parole_check)) {
                    ...
                    $holidays = DB::select("SELECT * FROM `holiday` WHERE date BETWEEN '$year-$month-01' and '$year-$month-31' and FIND_IN_SET('$employee->EmployeeId',employee_id)");//2;
                    $total_working_days = $month_days - count($holidays) - $weekend;
